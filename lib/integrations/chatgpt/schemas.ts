...
export const updateBlogPostSchema = z
  .object(updateBlogPostFields)
  .strict()
  .refine((value) => Object.keys(value).length > 0, "At least one blog field must be supplied");

export const updateBlogPostByNameSchema = z
  .object({
    blogName: text("blogName", 200),
    ...updateBlogPostFields,
  })
  .strict()
  .refine((value) => Object.keys(value).some((key) => key !== "blogName"), "At least one blog field must be supplied");
...
export type UpdateBlogPostByNameRequest = z.infer<typeof updateBlogPostByNameSchema>;
